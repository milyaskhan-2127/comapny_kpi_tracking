...
def setup_smtp_email_account():
    ...
    Secrets come exclusively from environment variables (see .env.example).
    Requires MAIL_PASSWORD to be set; the account is skipped (with a warning)
    when it is missing so no placeholder credentials are ever stored.
    """
    ...
    if not mail_password:
        print("  ⚠ Skipping SMTP Email Account — MAIL_PASSWORD is not set in the environment.")
        return {"success": False, "error": "MAIL_PASSWORD not configured"}
