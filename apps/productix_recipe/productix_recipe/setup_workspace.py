...
def _create_workspace():
    ...
    # The Instruction Room lives in a separate optional app (productix_instruction).
    # Only link it when the doctype actually exists on this site.
    if frappe.db.table_exists("Instruction Message"):
        shortcuts.append(
    ...
    if frappe.db.table_exists("Instruction Message"):
        links.append({"label": "Instruction Room", "link_to": "Instruction Message", "link_type": "DocType", "type": "Link"})
...
def _setup_custom_docperms():
    ...
    for dt, role, r, w, c, s, d in perms:
        if not frappe.db.exists("Role", role):
            continue
        # Skip doctypes not installed on this site (e.g. Instruction Message
        # when the optional productix_instruction app is absent).
