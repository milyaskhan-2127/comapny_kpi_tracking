"""
Productix Recipe — Scheduled Tasks (runs via ERPNext scheduler or manual trigger).

Registered in hooks.py scheduler_events. Both entry points start with
``require_module("recipe")`` so they are no-ops (with a clear error) when the
Recipe module is disabled — the documented standard pattern for productix
scheduled tasks.
"""
import frappe
...
def run_daily_inventory_scan():
    ...
    4. Log the result to the shared audit log (AI Agent Log)
    """
    ...
    try:
        ...
        try:
            from productix_core.productix_core.doctype.ai_agent_log.ai_agent_log import log_event as _log
...
def _create_in_app_alert(low_stock_items, expiring_batches):
    ...
    The Instruction module is optional — skip silently when it is not installed.
    """
