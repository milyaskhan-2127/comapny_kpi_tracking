...
Usage:
    docker compose exec backend bash -c "
        ...
        bench --site productix.local execute productix_recipe.setup_data.run
    "
"""
import frappe
from frappe.utils import today, add_days, now_datetime, nowdate, flt
from productix_recipe.setup_workspace import run as setup_workspace_run
from productix_core.utils.email_utils import setup_smtp_email_account
...
def _create_roles():
    ...
    # KPI roles are owned by the productix_kpi app — only seed them when the
    # KPI module is actually installed on this site.
    kpi_roles = [
    ...
    if frappe.db.table_exists("KPI Department"):
        roles = roles + kpi_roles
    for r in roles:
    ...
    print(f"  ✓ {len(roles)} Factory Roles configured." + (f" + {len(kpi_roles)} KPI Roles" if frappe.db.table_exists("KPI Department") else ""))
...
def _create_team_messages():
    # The Instruction Room module is optional — skip when not installed.
    if not frappe.db.exists("Doctype", "Instruction Message"):
        print("  ✓ Team Instruction Room announcements skipped (module not installed).")
        return
    messages = [
        {
