...
function load_backups(page, show_alert) {
    if (!page || !page.main || !page.main.length) return;
    frappe.call({
        method: 'productix_kpi.api.backup.get_backups_list',
        callback: function(r) {
            try {
...
function export_kpi_json_backup(page) {
    ...
    frappe.call({
        method: 'productix_kpi.api.backup.export_kpi_json_backup',
        freeze: true,
...
function create_backup(page, backup_type) {
    ...
    frappe.call({
        method: 'productix_kpi.api.backup.take_immediate_backup',
        args: {
...
function preview_restore_backup(page, filename) {
    frappe.call({
        method: 'productix_kpi.api.backup.preview_backup_file',
...
function execute_restore(page, filename) {
    frappe.call({
        method: 'productix_kpi.api.backup.restore_backup_file',
...
function confirm_delete_backup(page, filename) {
    frappe.confirm(
        ...
        function() {
            frappe.call({
                method: 'productix_kpi.api.backup.delete_backup_file',
                args: { filename: filename },
                freeze: true,
...
function handle_file_upload(page) {
    ...
    formData.append('cmd', 'productix_kpi.api.backup.upload_backup_file');
    ...
    xhr.open('POST', '/api/method/productix_kpi.api.backup.upload_backup_file', true);
