...
function load_dept(page, dept) {
    ...
    frappe.xcall("productix_kpi.kpi_tracking.api.dashboard.get_department_dashboard", {
        department: dept,
...
function render_dept(page, data, dept) {
    ...
    if (isEmployee) {
        page.main.find("#btn-send-escalation").on("click", function() {
            ...
            frappe.xcall("productix_kpi.kpi_tracking.services.alert_engine.send_employee_escalation_alert", {
                department: dept,
                message: msgBody,
...
function load_embedded_escalations(page, dept) {
    ...
    frappe.xcall("productix_kpi.kpi_tracking.services.alert_engine.get_department_escalations", {
        department: dept,
        limit: 10,
    }).then((escalations) => {
        ...
        if (isAdmin) {
            container.find(".ack-alert-btn").on("click", function() {
                ...
                frappe.xcall("productix_kpi.kpi_tracking.services.alert_engine.acknowledge_alert", { alert_name: name }).then(() => {
            ...
            container.find(".resolve-alert-btn").on("click", function() {
                const name = $(this).data("name");
                frappe.xcall("productix_kpi.kpi_tracking.services.alert_engine.resolve_alert", { alert_name: name }).then(() => {
...
function load_embedded_data_entry(page, dept) {
    frappe.xcall("productix_kpi.kpi_tracking.api.data_entry.get_pending_kpis", {
        department: dept,
        frequency: page._frequency,
    }).then((kpis) => {
        ...
        container.find(".dept-submit-btn").on("click", function () {
            ...
            frappe.xcall("productix_kpi.kpi_tracking.api.data_entry.submit_kpi_data", {
                kpi: kpiCode,
                department: deptCode,
...
function show_kpi_detail(page, kpi_code) {
    ...
    frappe.xcall("productix_kpi.kpi_tracking.api.dashboard.get_kpi_detail", {
        kpi_code: kpi_code,
        timeframe: page._timeframe,
        horizon: page._horizon,
    }).then((data) => {
