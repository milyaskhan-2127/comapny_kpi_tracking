...
Invoke via:
  bench --site productix.local execute productix_kpi.kpi_tracking.api._dev_checks.verify_state
  bench --site productix.local execute productix_kpi.kpi_tracking.api._dev_checks.run_acceptance
  bench --site productix.local execute productix_kpi.kpi_tracking.api._dev_checks.cleanse_test_data clear=1
...
def sync_workspace_from_json():
    ...
    path = frappe.get_app_path(
        "productix_kpi",
        "kpi_tracking", "workspace", "company_tracking_system",
        "company_tracking_system.json",
    )
...
def rename_110_debug():
    ...
    from productix_kpi.kpi_tracking.doctype.kpi_department.kpi_department import _normalize_code
...
def reset_patch_log():
    ...
    patch = "productix_kpi.migrations.productix.fix_kpi_department_records"
...
def backup_roundtrip():
    ...
    from productix_kpi.api.backup import (
...
def sync_pages_from_json(target=None, remove_stale=1):
    ...
    Usage: ... execute productix_kpi.kpi_tracking.api._dev_checks.sync_pages_from_json
    """
    import os
    import json as json_mod

    base = frappe.get_app_path("productix_kpi", "kpi_tracking", "page")
    names = []
    if target:
        names = [target]
    else:
...
def patch_status():
    ...
    try:
        from productix_kpi.kpi_tracking.doctype.kpi_department.kpi_department import _normalize_code as nc
...
def verify_state():
    ...
    _p("=== Registry ===")
    try:
        from productix_kpi.kpi_tracking.services import registry
...
def run_acceptance():
    ...
    from productix_kpi.kpi_tracking.api.user_management import save_user
    from productix_kpi.kpi_tracking.api import dashboard as dashboard_api
    from productix_kpi.kpi_tracking.api import machine as machine_api
...
def _users_internal():
    from productix_kpi.kpi_tracking.api.user_management import get_users
