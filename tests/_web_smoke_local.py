# -*- coding: utf-8 -*-
# Web-level gate + module smoke on the MIGRATED LIVE site (productix.local).
# Runs inside the backend container as plain python (no bench context).
# Usage: env/bin/python tests/_web_smoke_local.py [site]
...
SITE = sys.argv[1] if len(sys.argv) > 1 else "productix.local"
...
st, body = call(
    "/api/method/productix_instruction.instruction_room.doctype."
    "instruction_message.instruction_message.get_unread_count"
)
...
# --- legacy dotted route must no longer resolve (app retired on this site) ---
st, body = call("/api/method/productix.api.inventory.get_dashboard_kpis")
out.append(("LEGACY_ROUTE_CALL", st, body))
...
lines = [f"{name} | HTTP {code} | {b}" for name, code, b in out]
with open("/tmp/_web_smoke_local.txt", "w") as f:
    f.write("\n".join(lines) + "\n")
print("\n".join(lines))
